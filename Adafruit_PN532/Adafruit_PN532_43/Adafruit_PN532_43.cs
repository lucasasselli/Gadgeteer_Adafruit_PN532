...
namespace Gadgeteer.Modules.Luca_Sasselli
{
    ...
    public class Adafruit_PN532 : GTM.Module
    {
        ...
        static byte[] PN532_ACK = { 0x00, 0x00, 0xFF, 0x00, 0xFF, 0x00 };
        static byte[] PN532_FV_HEAD = { 0x00, 0xFF, 0x06, 0xFA, 0xD5, 0x03 };
        ...
        public uint GetFirmwareVersion() {
            ...
            uint version = (Utility.ExtractValueFromArray(response, 6, 4) >> 8) & 0xFF;
        ...
        private void Scan()
        {
            while (running)
            {
                byte[] result = ReadPassiveTargetID(PN532_MIFARE_ISO14443A, threadTimeout);
                if (result != null)
                {
                    string uid = "";
        ...
        private bool WriteGPIO(byte pinState)
        {
        ...
        // THIS IS AN HIDE METHOD: USE AT YOUR OWN RISK!
        private bool SetPassiveActivationRetries(uint number)
        {
        ...
        public byte[] ReadPassiveTargetID(byte baudrate, int timeout)
        {
            byte[] packet = {
